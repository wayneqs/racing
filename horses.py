...
def process_result(status):
	cache_directory = f"{os.environ['HOME']}/.racingcache"
	db = MongoClient().racing
	cursor = db.races.find({"status": status},no_cursor_timeout=True).batch_size(500)
	...
	for race in cursor:
		...
		if r.status_code == 200:
			...
			guid = uuid.uuid1()
			download_uri = f"{cache_directory}/{guid}"
...
def download_result_links():
	process_result("new")

def clean_race_id_url(url):
	return url.replace('amp;','').replace('&popup=yes','')
...
def get_result_links():
	...
	for d in daterange(date_or_none(current_pointer) or epoch, datetime.now()):
		print(d)
		r = get_url(f"http://www.racingpost.com/horses2/results/home.sd?r_date={d.strftime('%Y-%m-%d')}")
		if r.status_code == 200:
			print(f"SUCCESS: {r.url}")
			soup = BeautifulSoup(r.text, 'html.parser')
			...
			if links == None:
				...
				for l in links:
					...
					href = clean_race_id_url(l.get('href'))
					race_id = extract_race_id(href)
	...
	print('done with getting result links')
...
def extract_horses():
	print('extracting horses')
	db = MongoClient().racing
	cursor = db.races.find({"status": "downloaded", "data_extracted": { "$exists": False }}).batch_size(500)
	for race in cursor:
		uri = race["download_uri"]
		print(f"processing race {uri}")
		fo = open(uri, "r")
...
def warm_cache(db, cache):
	...
	if most_recent_rating != None:
		...
		print(f"adding {ratings.count()} ratings to the cache")
